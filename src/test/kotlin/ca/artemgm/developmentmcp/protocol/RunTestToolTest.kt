...
class RunTestToolTest {
    ...
    private val tool = RunTestTool(
        ...
        sourceReader = { null },
        module = stubModule,
        refreshAndWaitForSmartMode = { callOrder += "refreshAndWaitForSmartMode" },
    )
    ...
    inner class SmartModeOrdering {
        ...
        @Test
        fun `VFS refresh and smart mode wait precede config creation and execution`() {
            tool.runSilently(TestScope.CLASS, listOf("com.example.MyTest"))

            assertThat(callOrder.first()).isEqualTo("refreshAndWaitForSmartMode")
        }
    }
